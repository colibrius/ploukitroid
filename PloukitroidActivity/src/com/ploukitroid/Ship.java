...
public class Ship extends RectShape implements View.OnTouchListener
{
	...
	private Rect bullet;
	private int posX, posY, width, heigth, oldX, oldY, screenWidth, screenHeigth;
	private boolean isTouching, killAllTheFuckingEnemies;
	private View pView;
	public Ship(View pView, Context context, int type)
	{
		...
		Bitmap bShip = BitmapFactory.decodeResource(context.getResources(), path);
		...
		rectangle = new Rect(posX - width / 2, posY - heigth / 2, posX + width / 2, posY + heigth / 2);
	...
	public boolean getKillAllTheFuckingEnemies()
	{
		return killAllTheFuckingEnemies;
	}
	
	public void setKillAllTheFuckingEnemies(boolean killAllTheFuckingEnemies)
	{
		this.killAllTheFuckingEnemies = killAllTheFuckingEnemies;
	}
	...
	public boolean onTouch(View v, MotionEvent event)
	{
		for (int i = 0; i < event.getPointerCount(); i++)
		{
			...
			if (x < 0.8 * screenWidth)
				this.setPosition(this.posX, y, event);
			else
			{
				this.killAllTheFuckingEnemies = true;
				// bullet = new Rect(posX - width / 2, posY - heigth / 2, posX +
				// width / 2, posY + heigth / 2);
			}
		}
